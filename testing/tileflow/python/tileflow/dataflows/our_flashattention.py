...
def self_attention_dataflow_2levels(ctx, tQ, tK, tV, batch, num_heads, seq_len, hidden, define_tiling_space=True):
    ...
    tB_update_max = dir.Tensor([batch, num_heads, seq_len],
                    name="B", dtype="int16", ctx=ctx)
    tE_sum = dir.Tensor([batch, num_heads, seq_len],
                    name="B", dtype="int16", ctx=ctx)
    denom = dir.Tensor([batch, num_heads, seq_len],
                    name="denom", dtype="int16", ctx=ctx)
        ...
        with ctx.pipeline():
            with ctx.pipeline():
                with ctx.tile("L2", [b1, h1, m1], "Spatial"):
                    # Initialize running statistics before processing tiles
                    tB_update_max[b, h, m] = -32768  # Initialize to very negative value
                    denom[b, h, m] = 0  # Initialize denominator
                ...
                with ctx.tile("L2", [b1, h1, m1], "Spatial"):
                    with ctx.tile("L1", [b0, h0, m0], "Temporal"):
                        with ctx.tile("L1", [n1], "Spatial"):
                            with ctx.tile("L0", [n0], "Temporal"):
                                tG[b, h, m, n] = tG[b, h, m, n] / denom[b, h, m]
...
def get_tandem_self_attention_dataflow(levels, batch, num_heads, seq_len, hidden, define_tiling_space=True):
