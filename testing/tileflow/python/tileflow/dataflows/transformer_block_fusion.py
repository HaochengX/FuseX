                ...
                with ctx.tile("L1", [b0, m0, f2], "Temporal"):
                    with ctx.tile("L1", [f1], "Spatial"):
                        with ctx.tile("L0", [f0], "Temporal"):
                            ...
                            # Create zero by (x - x), then use (zero - x) to get -x
                            # This avoids unary negation and int-TensorView operations
                            tGELU[b, m, f] = tFFN1[b, m, f] / (1 + dir.exp((tFFN1[b, m, f] - tFFN1[b, m, f]) - tFFN1[b, m, f]))
