...
def self_attention_dataflow_2levels(ctx, tQ, tK, tV, batch, num_heads, seq_len, hidden, define_tiling_space=True):
    ...
    tH = dir.Tensor([batch, num_heads, seq_len, model_k],
                name="G", dtype="int16", ctx=ctx)
    tI = dir.Tensor([batch, num_heads, seq_len, model_k],
                name="G", dtype="int16", ctx=ctx)
        ...
        with ctx.pipeline():
                    ...
                    with ctx.tile("L1", [b0, h0, m0, l2], "Temporal"):
                            with ctx.tile("L1", [b0, h0, m0], "Spatial"):
                                with ctx.tile("L0", [l0], "Temporal"):
                                    ...
                                    # Compute exp and update running sum_exp
            ...
            with ctx.tile("L2", [b1, h1, m1], "Spatial"):
                with ctx.tile("L1", [b0, h0, m0, n2, l2], "Temporal"):
                    with ctx.tile("L1", [n1, l1], "Spatial"):
                        with ctx.tile("L0", [n0, l0], "Temporal"):
                            tI[b, h, m, n] = tI[b, h, m, n] + \
                                tG[b, h, m, l] * tH[b, h, l, n]
            with ctx.tile("L2", [b1, h1, m1], "Spatial"):
                with ctx.tile("L1", [b0, h0, m0, n2, l2], "Temporal"):
                    with ctx.tile("L1", [n1, l1], "Spatial"):
                        with ctx.tile("L0", [n0, l0], "Temporal"):
                            tJ_sum[b, h, m, n] = tJ_sum[b, h, m, n] + tI[b, h, m, n]
                            tK[b,h,m,n] = tk[b,h,m,n] - tJ_sum[b,h,m,n]
...
def get_ourwork_2_self_attention_dataflow(levels, batch, num_heads, seq_len, hidden, define_tiling_space=True):
