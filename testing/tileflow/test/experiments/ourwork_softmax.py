...
def replay(logfile, hw_id, batch, num_heads, seq_len, hidden, metric_type, debug=False, resource_check=True, define_tiling_space=True):
    ...
    # hw = [(2, get_edge_small()), (2, get_edge_large()), (3, get_cloud_small()), (3, get_cloud_large())]
    ...
    hw_config = acc.tileflow_accelerator_generator(
        hw[hw_id][1])
    dataflow = td.get_ourwork_softmax_self_attention_dataflow(
        levels, batch, num_heads, seq_len, hidden, define_tiling_space=define_tiling_space)
...
if __name__ == "__main__":
    ...
    if args.inference:
        ...
        for shape in shapes[args.begin:args.begin+args.number]:
            ...
            seq_len = shape[1]
            hidden = shape[2]
            for i, (levels, hw) in enumerate(zip([2, 2, 2, 2, 2, 2, 2, 2], [get_edge_small_32_4(), 
                                                          get_edge_small_32_8(),
                                                          get_edge_small_32_12(),
                                                          get_edge_small_32_16(),
                                                          get_edge_small_32_20(),
                                                          get_edge_small_32_24(),
                                                          get_edge_small_32_28(),
                                                          get_edge_small()])):
                print(
